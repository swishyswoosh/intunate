...
public class LibraryPrinter {
	...
	private void doPrintLibrary(final LibraryObject object) {
		if (object.getParent() == null) {
			/* log.debug */ System.out.println("Root - " + object.getType() + "[");
		}
		try {
			for (final LibraryObject obj : object.getChildren()) {
				/* log.debug */ System.out.println(obj.getType());
				if (obj.hasChildren()) {
					/* log.debug */ System.out.println("Children{");
					doPrintLibrary(obj);
					/* log.debug */ System.out.println("}");
				} else if (obj instanceof TextValue) {
					TextValue textVal = (TextValue) obj;
					/* log.debug */ System.out.println("#cData[" + textVal.getTextValue() + "]");
				}
			}
